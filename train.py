# -*- coding: utf-8 -*-
#
# train.py - train a new model on one or across multiple GPUs.
#
...
import xgcn
from xgcn import data_utils


def add_train_args(parser):
    parser.add_argument("--align-suffix",
                        metavar="FP",
                        default=None,
...
def main(args, init_distributed=False):
    ...
    # Load dataset (we load training data below, based on the latest checkpoint)
    ...
    # Build trainer
    trainer = Trainer(args, task, model, criterion, dummy_batch=True)
    ...
    _, epoch_itr = data_utils.load_checkpoint(args, task, model, trainer)
    ...
    while lr > args.min_lr and epoch < max_epoch and trainer.get_num_updates(
    ) < max_update:
        # train for one epoch
...
def train(args, trainer, task, epoch_itr):
    ...
    itr = epoch_itr.next_epoch_itr(
        fix_batches_to_gpus=args.fix_batches_to_gpus, shuffle=False)
    ...
    for i, samples in enumerate(progress, start=epoch_itr.iterations_in_epoch):
        ...
        if (not args.disable_validation and args.save_interval_updates > 0
            ...
            valid_losses = validate(args, trainer, task, epoch_itr,
                                    valid_subsets)
            checkpoint_utils.save_checkpoint(args, trainer, None,
                                             valid_losses[0])
